...
import com.p2p.tormaster.service.MasterBackupManager
import com.p2p.tormaster.service.MasterWalletManager
import com.p2p.tormaster.service.NetworkTimeFetcher
import com.p2p.tormaster.service.TorManager
import com.p2p.tormaster.service.TorState
import com.p2p.tormaster.service.TotpManager
...
class MainActivity : ComponentActivity() {
    ...
    private val cameraPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { if (it) currentScreen = MasterScreen.Scanner }
    ...
    private fun AppContent() {
        ...
        ModalNavigationDrawer(drawerContent = {
            MasterDrawer(ds, sc)
        }, drawerState = ds, gesturesEnabled = currentScreen !is MasterScreen.Auth && currentScreen !is MasterScreen.Scanner) {
            Scaffold(
                topBar = { if (currentScreen !is MasterScreen.Auth && currentScreen !is MasterScreen.Scanner) MasterTopBar(ds, sc) },
            ) { padding ->
                Box(Modifier.padding(padding)) {
                    when (currentScreen) {
                        ...
                        is MasterScreen.Scanner ->
                            MasterQRScannerScreen({
                                scannedUtenteId = it.removePrefix("http://").removePrefix("https://").removeSuffix("/")
                                currentScreen = MasterScreen.Generator
                            }, { currentScreen = MasterScreen.Generator })
    ...
    private fun handleAuth(
        ...
        if (masterPasswordHash == null) {
            ...
            failedAttempts++
            getSharedPreferences("master_prefs", Context.MODE_PRIVATE).edit().putInt("failed_attempts", failedAttempts).apply()
    ...
    private fun handleExport(u: Uri) {
        try {
            ...
            getSharedPreferences("master_prefs", Context.MODE_PRIVATE).edit().putString("last_backup_uri", u.toString()).apply()
            contentResolver.openOutputStream(u)?.use {
                it.write(
                    MasterBackupManager(
                        this,
                    ).createEncryptedBackupJson(currentSeed, getOrCreateSalt(), walletManager.getBalance()).toByteArray(),
                )
            }
    ...
    private fun handleImport(u: Uri) {
        try {
            ...
            if (pkg != null && MasterBackupManager(this).restoreFromEncryptedBackup(pkg, currentSeed) != null) {
                finish()
    ...
    private fun loadPreferences() {
        val p = getSharedPreferences("master_prefs", Context.MODE_PRIVATE)
    ...
    private fun getOrCreateSalt(): ByteArray {
        val p = getSharedPreferences("master_secure_prefs", Context.MODE_PRIVATE)
    ...
    private fun performWipe() {
        getSharedPreferences("master_prefs", Context.MODE_PRIVATE).edit().clear().apply()
        getSharedPreferences("master_wallet_prefs", Context.MODE_PRIVATE).edit().clear().apply()
        try {
    ...
    @Composable
    private fun MasterDrawer(
        ds: DrawerState,
        sc: CoroutineScope,
    ) {
        ModalDrawerSheet {
            Spacer(Modifier.height(12.dp))
            Text("MASTER WALLET", Modifier.padding(16.dp), Color.Cyan, fontWeight = FontWeight.Bold)
            NavigationDrawerItem({ Text("WALLET") }, currentScreen == MasterScreen.Wallet, {
                currentScreen = MasterScreen.Wallet
                sc.launch { ds.close() }
            }, icon = { Icon(Icons.Default.AccountBalanceWallet, null) })
            NavigationDrawerItem({ Text("SBLOCCA") }, currentScreen == MasterScreen.Generator, {
                currentScreen = MasterScreen.Generator
                sc.launch { ds.close() }
            }, icon = { Icon(Icons.Default.VpnKey, null) })
            NavigationDrawerItem({ Text("BACKUP") }, currentScreen == MasterScreen.SeedBackup, {
                currentScreen = MasterScreen.SeedBackup
                sc.launch { ds.close() }
    ...
    @OptIn(ExperimentalMaterial3Api::class)
    @Composable
    private fun MasterTopBar(
        ds: DrawerState,
        sc: CoroutineScope,
    ) {
        CenterAlignedTopAppBar(title = {
            Text("TOR MASTER")
        }, navigationIcon = { IconButton({ sc.launch { ds.open() } }) { Icon(Icons.Default.Menu, null) } })
    }
        ...
        Box(Modifier.fillMaxSize().background(Color.Black), Alignment.Center) {
            Column(Modifier.padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                OutlinedTextField(p, {
                    ...
                    IconButton({ v = !v }) { Icon(if (v) Icons.Default.Visibility else Icons.Default.VisibilityOff, null) }
                })
                if (masterPasswordHash == null) {
                    OutlinedTextField(c, {
                        c = it
    ...
    @Composable fun WalletScreen() {
        ...
        val tor by torManager.torState.collectAsState()
        val myO = (tor as? TorState.Running)?.onionAddress ?: "..."
        Column(Modifier.fillMaxSize().background(Color.Black).padding(24.dp), horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Card(Modifier.padding(top = 16.dp)) {
                Column(Modifier.padding(16.dp)) {
                    Text("ID MASTER:", color = Color.Cyan)
                    Text(myO, color = Color.White, fontSize = 12.sp)
                }
            }
    ...
    @Composable fun GeneratorScreen() {
        var uid by remember { mutableStateOf(scannedUtenteId) }
        var d by remember { mutableIntStateOf(30) }
        var code by remember { mutableStateOf<String?>(null) }
        Column(Modifier.fillMaxSize().background(Color.Black).padding(24.dp)) {
            OutlinedTextField(uid, {
                uid = it
            }, label = {
                Text("ID Utente")
            }, trailingIcon = {
                IconButton({ cameraPermissionLauncher.launch(Manifest.permission.CAMERA) }) { Icon(Icons.Default.QrCodeScanner, null) }
            })
            Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween, Alignment.CenterVertically) {
                Text("GIORNI: $d", color = Color.White)
                Row {
    ...
    @Composable fun RechargeScreen() {
        ...
        Column(Modifier.fillMaxSize().background(Color.Black).padding(24.dp)) {
            ...
            }, Modifier.fillMaxWidth()) { Text("APPLICA") }
        }
    }

    @Composable fun MasterInfoScreen() {
        Column(Modifier.fillMaxSize().background(Color.Black).padding(24.dp)) {
            Text("INFO MASTER", color = Color.Cyan)
            Text("P2P Tor pura.", color = Color.White)
        }
    }
    ...
    private fun autoBackup() {
        if (isAutoBackupEnabled && currentSeed.isNotEmpty()) {
            getSharedPreferences("master_prefs", Context.MODE_PRIVATE).getString("last_backup_uri", null)?.let {
                try {
