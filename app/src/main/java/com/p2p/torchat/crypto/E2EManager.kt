...
class E2EManager {
    companion object {
        ...
        fun deleteMasterKey() { KeyStore.getInstance(ANDROID_KEYSTORE).apply { load(null); deleteEntry(MASTER_KEY_ALIAS) } }
        fun getFingerprint(pk: PublicKey): String = MessageDigest.getInstance("SHA-256").digest(pk.encoded).joinToString("") { "%02X".format(it) }.chunked(4).joinToString("-")

        fun deriveKeyArgon2id(secret: String, salt: ByteArray): SecretKeySpec {
            val res = argon2.hash(Argon2Mode.ARGON2_ID, secret.toByteArray(StandardCharsets.UTF_8), salt, 2, 65536, 1, 32)
            return SecretKeySpec(res.rawHashAsByteArray(), "AES")
        }

        fun generateECDHKeyPair(): KeyPair { val kg = KeyPairGenerator.getInstance("EC"); kg.initialize(256); return kg.generateKeyPair() }
        fun getSharedSecret(priv: PrivateKey, pub: PublicKey): SecretKeySpec { val ka = KeyAgreement.getInstance("ECDH"); ka.init(priv); ka.doPhase(pub, true); return SecretKeySpec(MessageDigest.getInstance("SHA-256").digest(ka.generateSecret()), "AES") }
        fun stringToPublicKey(b64: String): PublicKey = KeyFactory.getInstance("EC").generatePublic(X509EncodedKeySpec(Base64.getDecoder().decode(b64)))
        ...
        fun signHandshake(data: String, priv: PrivateKey): String { val sig = Signature.getInstance("SHA256withECDSA"); sig.initSign(priv); sig.update(data.toByteArray(StandardCharsets.UTF_8)); return Base64.getEncoder().encodeToString(sig.sign()) }
        fun verifyHandshake(data: String, sigStr: String, pub: PublicKey): Boolean = try { val sig = Signature.getInstance("SHA256withECDSA"); sig.initVerify(pub); sig.update(data.toByteArray(StandardCharsets.UTF_8)); sig.verify(Base64.getDecoder().decode(sigStr)) } catch (e: Exception) { false }
