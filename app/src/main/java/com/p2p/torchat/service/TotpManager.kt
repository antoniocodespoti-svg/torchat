...
class TotpManager {
    ...
    fun findMatchingClientDuration(
        ...
        for (days in 1..1000) {
            if (inputCode == generateClientCode(onionAddress, networkTime, days)) return days
            if (inputCode == generateClientCode(onionAddress, networkTime - TIME_STEP_MS, days)) return days
        }
        return null
    }
