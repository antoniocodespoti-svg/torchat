...
object TotpManager {
    ...
    /**
     * Client-side validation: Tries to find which duration matches the input code.
     */
    fun findMatchingClientDuration(
        inputCode: String,
        onionAddress: String,
        networkTime: Long,
    ): Int? {
        ...
        val mac = Mac.getInstance("HmacSHA1")
        for (days in 1..1000) {
            if (inputCode == generateClientCodeInternal(digest, mac, T2_SECRET, onionAddress, networkTime, days, DIGITS_CLIENT)) return days
            if (inputCode == generateClientCodeInternal(digest, mac, T2_SECRET, onionAddress, networkTime - TIME_STEP_MS, days, DIGITS_CLIENT)) return days
        }
        return null
    }
    ...
    fun findMatchingMasterDuration(
        ...
        val digest = MessageDigest.getInstance("SHA-256")
        val mac = Mac.getInstance("HmacSHA1")
        // Checking common packs first for performance
    ...
    private fun generateClientCodeInternal(digest: MessageDigest, mac: Mac, baseSecret: String, onion: String, time: Long, duration: Int, digits: Int): String {
