...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var myKeyPair: KeyPair; private val sessionKeys = mutableStateMapOf<String, javax.crypto.SecretKey>()
    private val handshakeLoading = mutableStateMapOf<String, Boolean>(); private val messageCounters = mutableMapOf<String, Int>()
    private var noiseJob: Job? = null; private val peersList = mutableStateListOf<Peer>()
    private val messagesMap = mutableStateMapOf<String, MutableList<Message>>(); private val unreadCounts = mutableStateMapOf<String, Int>()
    private var currentScreen by mutableStateOf<Screen>(Screen.Auth); private var isAuthenticated by mutableStateOf(false)
    private var isTermsAccepted by mutableStateOf(false); private var savedPasswordHash by mutableStateOf<String?>(null)
    private var failedAttempts by mutableStateOf(0); private var currentSeed by mutableStateOf<List<String>>(emptyList())
    private var myAlias by mutableStateOf("Amico"); private var isDarkTheme by mutableStateOf(true)
    private var isAutoBackupEnabled by mutableStateOf(false); private var isAvailable by mutableStateOf(false)
    private var expiryDate by mutableStateOf(0L); private var expiryAlertMessage by mutableStateOf<String?>(null)
    ...
    private var activeChatPeer: Peer? = null; private var activeChatMessages: MutableList<Message>? = null
    ...
    private fun AppContent() {
        val torState by torManager.torState.collectAsState(); val myOnion = (torState as? TorState.Running)?.onionAddress ?: ""
        ...
        Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
            when (val screen = currentScreen) {
                ...
                is Screen.Settings -> SettingsScreen({ currentScreen = Screen.SeedBackup }, { currentScreen = Screen.SeedRestore }, isAutoBackupEnabled, { isAutoBackupEnabled = it; saveAutoBackupPreference(it) }, { currentScreen = Screen.ChangePassword }, { currentScreen = Screen.Subscription }, expiryDate, { currentScreen = Screen.Info }, { currentScreen = Screen.TermsOfUse }, { currentScreen = Screen.Home })
    ...
    private fun handleSubscription(code: String, onion: String) { CoroutineScope(Dispatchers.IO).launch { val netTime = timeFetcher.fetchTimeViaTor() ?: System.currentTimeMillis(); val matchedDays = totpManager.findMatchingClientDuration(code, onion, netTime); if (matchedDays != null) { expiryDate = maxOf(netTime, expiryDate) + (matchedDays.toLong() * 24 * 60 * 60 * 1000); saveExpiryDate(expiryDate); withContext(Dispatchers.Main) { currentScreen = Screen.Home; Toast.makeText(this@MainActivity, "OK", Toast.LENGTH_SHORT).show() } } } }
    ...
    private fun handleHandshakeReceived(p: NetworkPayload) {
        try {
            ...
            val peerIdentityKey = E2EManager.stringToPublicKey(pts[2])
            ...
            val peerEphemeralPubKey = E2EManager.stringToPublicKey(pts[0])
            val sKBytes = com.p2p.torchat.crypto.HKDF.deriveKey(E2EManager.getSharedSecret(myKeyPair.private, peerEphemeralPubKey).encoded, null, "SessionKey".toByteArray(), 32)
    ...
    private fun handleAcceptConnection(req: NetworkPayload) {
        ...
        try { val peerEphemeralPubKey = E2EManager.stringToPublicKey(p[0]); val sKBytes = com.p2p.torchat.crypto.HKDF.deriveKey(E2EManager.getSharedSecret(myKeyPair.private, peerEphemeralPubKey).encoded, null, "SessionKey".toByteArray(), 32); sessionKeys[req.senderOnion] = SecretKeySpec(sKBytes, "AES") } catch (e: Exception) { }
    ...
    private fun getOrCreateSalt(): ByteArray {
        val p = getSharedPreferences("secure_prefs_salt", Context.MODE_PRIVATE); val sEnc = p.getString("install_salt_enc", null)
    ...
    private fun initiateHandshake(p: Peer) {
        if (p.handshakePublicKey.isEmpty()) return; if (handshakeLoading[p.onionAddress] == true) return; handshakeLoading[p.onionAddress] = true;
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val eKP = E2EManager.generateECDHKeyPair(); val eKStr = E2EManager.publicKeyToString(eKP.public)
                val sig = E2EManager.signHandshake(eKStr, myKeyPair.private)
                val myIDStr = E2EManager.publicKeyToString(myKeyPair.public)
                ...
                if (res.isSuccess) { val pPK = E2EManager.stringToPublicKey(p.handshakePublicKey); val sS = E2EManager.getSharedSecret(eKP.private, pPK); sessionKeys[p.onionAddress] = SecretKeySpec(com.p2p.torchat.crypto.HKDF.deriveKey(sS.encoded, null, "SessionKey".toByteArray(), 32), "AES") }
