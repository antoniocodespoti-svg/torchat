...
class MainActivity : ComponentActivity() {
    ...
    private val sessionKeys = mutableStateMapOf<String, SecretKey>()
    ...
    private val pickImageLauncher =
        registerForActivityResult(ActivityResultContracts.PickVisualMedia()) { u -> u?.let { handleMediaPick(it, true) } }
    private val pickFileLauncher =
        registerForActivityResult(ActivityResultContracts.GetContent()) { u -> u?.let { handleMediaPick(it, false) } }
    private val exportBackupLauncher =
        registerForActivityResult(ActivityResultContracts.CreateDocument("application/json")) { u -> u?.let { handleExport(it) } }
    private val importBackupLauncher =
        registerForActivityResult(ActivityResultContracts.OpenDocument()) { u -> u?.let { handleImport(it) } }
    private val orbotLauncher =
        registerForActivityResult(
            ActivityResultContracts.StartActivityForResult(),
        ) { res ->
            if (res.resultCode == RESULT_OK) {
                res.data?.getStringExtra("onion_address")?.let {
    ...
    private fun AppContent() {
        ...
        Surface(Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
            when (val screen = currentScreen) {
                is Screen.Auth ->
                    AuthScreen(mode = if (savedPasswordHash == null) AuthMode.CREATE else AuthMode.LOGIN, attemptsLeft = 3 - failedAttempts, onAuthSuccess = {
                        handleAuthResult(it)
                    })
                is Screen.Subscription -> SubscriptionScreen(myOnion, { handleSubscription(it, myOnion) })
                is Screen.Home ->
                    HomeScreen(
                        torState, myOnion, myAlias,
                        E2EManager.publicKeyToString(
                            myKeyPair.public,
                        ),
                        isDarkTheme, isAvailable, expiryDate, peersList, unreadCounts, peerToConfirmWithKey, {
                            isDarkTheme = !isDarkTheme
                            saveThemePreference(isDarkTheme)
                        }, {
                            isAvailable = !isAvailable
                            saveAvailabilityPreference(isAvailable)
                            broadcastMyStatus(isAvailable, peersList)
                        }, {
                            myAlias = it
                            saveMyAlias(it)
                            ...
                            currentScreen = Screen.QRCode
                        }, { currentScreen = Screen.QRScanner }, { currentScreen = Screen.Settings }, { torManager.setTorRunning(it) }, {
                            peersList.remove(it)
                            savePeers(peersList)
                        }, { peerToConfirmWithKey = null },
                    )
                is Screen.Chat -> ChatScreenContent(screen.peer)
                is Screen.QRCode ->
                    QRCodeScreen(
                        myOnion,
                        myAlias,
                        E2EManager.publicKeyToString(myKeyPair.public),
                        { currentScreen = Screen.Home },
                    )
                is Screen.Verification ->
                    VerificationScreen(
                        screen.peer,
                        { handleVerifyPeer(screen.peer) },
                        { currentScreen = Screen.Chat(screen.peer) },
                    )
                is Screen.QRScanner -> ClientQRScannerScreen({ handleQRScan(it) }, { currentScreen = Screen.Home })
                is Screen.Settings ->
                    SettingsScreen({ currentScreen = Screen.SeedBackup }, { currentScreen = Screen.SeedRestore }, isAutoBackupEnabled, {
                        isAutoBackupEnabled = it
                        saveAutoBackupPreference(it)
                    }, {
                        currentScreen = Screen.ChangePassword
                    }, {
                        currentScreen = Screen.Subscription
                    }, expiryDate, { currentScreen = Screen.Info }, { currentScreen = Screen.TermsOfUse }, { currentScreen = Screen.Home })
                is Screen.TermsOfUse ->
                    TermsOfUseScreen(isViewOnly = isTermsAccepted, onAccept = {
                        handleTermsAccept()
                    }, onBack = { currentScreen = Screen.Settings })
                is Screen.Info -> InfoScreen({ currentScreen = Screen.Settings })
                is Screen.ChangePassword ->
                    AuthScreen(AuthMode.CHANGE, onAuthSuccess = {
                        handleChangePassword(it)
                    }, onBack = { currentScreen = Screen.Settings })
                is Screen.SeedBackup -> SeedBackupContent()
                is Screen.SeedRestore ->
                    SeedScreen(
                        SeedMode.INPUT,
                        emptyList(),
                        { handleSeedRestore(it) },
    ...
    @Composable private fun ChatScreenContent(p: Peer) {
        ...
        ChatScreen(p, ms, handshakeLoading[p.onionAddress] ?: false, { sendMessage(p, it, ms) }, {
            activeChatPeer = p
            activeChatMessages = ms
            pickImageLauncher.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageOnly))
        }, {
            activeChatPeer = p
            activeChatMessages = ms
            pickFileLauncher.launch("*/*")
        }, { f, b -> saveAttachmentToExternalStorage(f, b) }, {
            handleDeleteSession(p, ms)
        }, { currentScreen = Screen.Verification(p) }, { currentScreen = Screen.Home })
    }
    ...
    @Composable private fun SeedBackupContent() {
        ...
        SeedScreen(SeedMode.DISPLAY, currentSeed, { exportBackupLauncher.launch("torchat_backup.json") }, {
            val p = getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE)
            if (isAuthenticated && (currentSeed.isNotEmpty() || p.getString("persistent_backup_key", null) != null)) currentScreen = Screen.Home else currentScreen = Screen.Settings
        }, { currentScreen = Screen.Home }, { handleRemoveSeed() })
    }

    private fun handleAuthResult(password: String): Boolean {
        if (password == "WIPE_NOW") {
            performWipe()
            return false
        }
        if (password.startsWith("CREATE:")) {
            try {
                val p = password.removePrefix("CREATE:").split("|")
                savePasswordHash(E2EManager.hashPassword(p[0]))
                savedPasswordHash = E2EManager.hashPassword(p[0])
                torManager.setTorRunning(p[1])
                completeAuth()
                currentScreen = Screen.SeedBackup
                return true
            } catch (
                e: Exception,
            ) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (matchedDays != null) {
                ...
                withContext(Dispatchers.Main) {
                    currentScreen = Screen.Home
                    Toast.makeText(this@MainActivity, "ESTESO!", Toast.LENGTH_LONG).show()
                }
            }
        }
    }

    private fun handleAddPeer(
        alias: String,
        onion: String,
        pubKey: String,
    ) {
        ...
        if (peersList.any { it.onionAddress == clean }) {
            ...
            peersList.add(Peer(clean, alias, pubKey))
            savePeers(peersList)
        }
    }
    ...
    private fun handleSelectPeer(peer: Peer) {
        ...
        if (!sessionKeys.containsKey(peer.onionAddress)) initiateHandshake(peer)
        currentScreen = Screen.Chat(peer)
    }

    private fun handleVerifyPeer(peer: Peer) {
        val idx = peersList.indexOf(peer)
        if (idx != -1) {
            peersList[idx] = peer.copy(isVerified = true)
            savePeers(peersList)
        }
        currentScreen = Screen.Home
    }
    ...
    private fun handleTermsAccept() {
        ...
        ).edit().putBoolean("is_terms_accepted", true).putLong("terms_accepted_timestamp", System.currentTimeMillis()).apply()
        currentScreen = Screen.Auth
    }

    private fun handleChangePassword(data: String): Boolean {
        val p = data.removePrefix("VERIFY:").split("|")
        if (E2EManager.hashPassword(p[0]) == savedPasswordHash) {
    ...
    private fun handleDeleteSession(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            p2pMessenger.sendPayloadOverTor(
                peer.onionAddress,
                NetworkPayload(
                    PayloadType.SESSION_TERMINATE,
                    (torManager.torState.value as? TorState.Running)?.onionAddress ?: "",
                    peer.onionAddress,
                    "TERMINATE",
                ),
            )
        ...
        messages.clear()
        sessionKeys.remove(peer.onionAddress)
    ...
    private fun handleRemoveSeed() {
        if (currentSeed.isNotEmpty()) {
            ...
            val key = MnemonicManager.deriveKeyFromMnemonic(currentSeed, salt)
            ...
            getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().apply {
                putString("persistent_backup_key", keyB64)
                remove("saved_seed")
                apply()
            }
    ...
    private fun handleExport(uri: Uri) {
        try {
            ...
            getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putString("last_backup_uri", uri.toString()).apply()
            val salt = getOrCreateSalt()
            contentResolver.openOutputStream(
                uri,
            )?.use { it.write(backupManager.createEncryptedBackupJson(currentSeed, salt).toByteArray()) }
            if (currentScreen == Screen.SeedBackup) currentScreen = Screen.Home else currentScreen = Screen.Settings
        } catch (e: Exception) {
        }
    }
    ...
    private fun handleIncomingPayload(payload: NetworkPayload) {
        when (payload.type) {
                ...
                if (isAvailable) {
                    try {
                        ...
                        if (sK != null) {
                            ...
                            val c = (messageCounters[payload.senderOnion] ?: 0) + 1
                            messageCounters[payload.senderOnion] = c
                            if (c >= 5) triggerSessionRotation(peersList.find { it.onionAddress == payload.senderOnion } ?: return)
                            ...
                            notificationHelper.showChatNotification(
                                peersList.find { it.onionAddress == payload.senderOnion }?.alias ?: "Peer",
                                dec,
                            )
                        }
                    } catch (e: Exception) {
            ...
            PayloadType.SESSION_HANDSHAKE -> handleHandshakeReceived(payload)
            PayloadType.SESSION_TERMINATE -> {
                sessionKeys.remove(payload.senderOnion)
            ...
            PayloadType.PING -> {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    p2pMessenger.sendPayloadOverTor(
                        payload.senderOnion,
                        NetworkPayload(PayloadType.PONG, my, payload.senderOnion, if (isAvailable) "ONLINE" else "OFFLINE"),
                        timeoutMs = 10000,
                    )
                }
            }
            PayloadType.PONG -> {
                ...
                if (idx != -1) peersList[idx] = peersList[idx].copy(isOnline = payload.payloadData == "ONLINE", lastSeenTimestamp = System.currentTimeMillis())
            }
            else -> {}
        }
    }
    ...
    private fun handleHandshakeReceived(p: NetworkPayload) {
        try {
            ...
            if (existing.handshakePublicKey.isNotEmpty() && existing.handshakePublicKey != pts[2]) return
            val pK = E2EManager.stringToPublicKey(pts[0])
            val sK =
                com.p2p.torchat.crypto.HKDF.deriveKey(
                    E2EManager.getSharedSecret(myKeyPair.private, pK).encoded,
                    null,
                    "SessionKey".toByteArray(),
                    32,
                )
            sessionKeys[p.senderOnion] = SecretKeySpec(sK, "AES")
            handshakeLoading[p.senderOnion] = false
        } catch (e: Exception) {
        }
    }

    @Composable private fun HandleExpiryCheck(tor: TorState) {
        LaunchedEffect(isAuthenticated, tor, currentScreen) {
            if (isAuthenticated && tor is TorState.Running) {
    ...
    @Composable private fun DialogsContent() {
        pendingConnectionRequest?.let {
                req ->
            AlertDialog({
                pendingConnectionRequest = null
            }, {
                Button({
                    handleAcceptConnection(req)
                }) {
                    Text("OK")
                }
            }, dismissButton = {
            ...
            }, title = { Text("RICHIESTA") }, text = { Text("ID: ${req.senderOnion.take(15)}...") })
        }
        expiryAlertMessage?.let {
                msg ->
            AlertDialog({
                expiryAlertMessage = null
            }, { Button({ expiryAlertMessage = null }) { Text("OK") } }, title = { Text("AVVISO") }, text = { Text(msg) })
        }
    }

    private fun handleAcceptConnection(req: NetworkPayload) {
        val p = req.payloadData.split("|")
        val newP = Peer(req.senderOnion, "Nuovo Amico", if (p.size >= 3) p[2] else "")
        peersList.add(newP)
        savePeers(peersList)
        try {
            val pK = E2EManager.stringToPublicKey(p[0])
            val sK =
                com.p2p.torchat.crypto.HKDF.deriveKey(
                    E2EManager.getSharedSecret(myKeyPair.private, pK).encoded,
                    null,
            ...
            sessionKeys[req.senderOnion] = SecretKeySpec(sK, "AES")
        } catch (
            e: Exception,
        ) {
        }
    ...
    private fun loadPreferences() {
        ...
        currentScreen = if (!isTermsAccepted) Screen.TermsOfUse else Screen.Auth
        expiryDate = p.getLong("account_expiry_date", 0L)
        loadOrGenerateIdentityKeys(p)
        peersList.clear()
        peersList.addAll(loadPeers())
    }
    ...
    private fun sendMessage(
        ...
        val encrypted = E2EManager.encrypt(content, sK)
        val my = (torManager.torState.value as? TorState.Running)?.onionAddress ?: ""
        val msg =
            Message(
                senderOnion = my,
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (res.isSuccess) {
                val c = (messageCounters[peer.onionAddress] ?: 0) + 1
                messageCounters[peer.onionAddress] = c
                if (c >= 5) triggerSessionRotation(peer)
            }
    ...
    private fun handleMediaPick(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            b?.let {
                ...
                val d = mediaManager.getFileDetails(u)
                sendMessage(p, b64, m)
            }
        }
    }
    ...
    private fun saveAttachmentToExternalStorage(
        ...
        try {
            ...
            val v = android.content.ContentValues().apply { put(android.provider.MediaStore.MediaColumns.DISPLAY_NAME, f) }
            val uri =
    ...
    private fun autoBackup() {
        if (isAutoBackupEnabled && currentSeed.isNotEmpty()) {
            getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).getString("last_backup_uri", null)?.let {
                try {
                    contentResolver.openOutputStream(
                        it.toUri(),
                    )?.use { os -> os.write(backupManager.createEncryptedBackupJson(currentSeed, getOrCreateSalt()).toByteArray()) }
                } catch (e: Exception) {
                }
            }
        }
    ...
    private fun savePeers(peers: List<Peer>) {
        val p = getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE)
        val j = Gson().toJson(peers.toList())
        try {
            val h = p.getString("app_password_hash", null)
    ...
    private fun getOrCreateSalt(): ByteArray {
        ...
        val s = p.getString("install_salt_enc", null)
        return if (s != null) {
            try {
                Base64.getDecoder().decode(E2EManager.decryptWithHardwareKey(s))
            } catch (e: Exception) {
    ...
    private fun startNoiseGenerator() {
        ...
        noiseJob =
            CoroutineScope(Dispatchers.IO).launch {
                while (true) {
                    delay(Random.nextLong(300_000, 900_000))
                    if (isAvailable && peersList.isNotEmpty()) {
                        val p = peersList.random()
                        val my = (torManager.torState.value as? TorState.Running)?.onionAddress ?: ""
                        if (my.isNotEmpty()) p2pMessenger.sendPayloadOverTor(p.onionAddress, NetworkPayload(type = PayloadType.DUMMY_NOISE, senderOnion = my, recipientOnion = p.onionAddress, payloadData = "NOISE_" + UUID.randomUUID()))
                    }
                }
            }
    }

    private fun triggerSessionRotation(p: Peer) {
        Log.d(TAG, "Rotating key for ${p.alias}")
        messageCounters[p.onionAddress] = 0
        initiateHandshake(p)
    }

    private fun saveMyAlias(a: String) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putString("my_alias", a).apply()
    }

    private fun saveThemePreference(d: Boolean) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putBoolean("is_dark_theme", d).apply()
    }

    private fun saveAvailabilityPreference(a: Boolean) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putBoolean("is_available", a).apply()
    }

    private fun savePasswordHash(h: String) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putString("app_password_hash", h).apply()
    }

    private fun saveFailedAttempts(a: Int) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putInt("failed_attempts", a).apply()
    }

    private fun saveExpiryDate(d: Long) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putLong("account_expiry_date", d).apply()
        autoBackup()
    }

    private fun saveAutoBackupPreference(e: Boolean) {
        getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putBoolean("is_auto_backup_enabled", e).apply()
    }
    ...
    private fun loadOrGenerateIdentityKeys(p: android.content.SharedPreferences) {
        ...
        if (pub != null && privEnc != null) {
            try {
                val priv = E2EManager.decryptWithHardwareKey(privEnc)
                val f = KeyFactory.getInstance("EC")
                myKeyPair = KeyPair(f.generatePublic(X509EncodedKeySpec(Base64.getDecoder().decode(pub))), f.generatePrivate(PKCS8EncodedKeySpec(Base64.getDecoder().decode(priv))))
            } catch (e: Exception) {
                generateNewIdentityKeys(p)
            }
        } else {
            generateNewIdentityKeys(p)
        }
    ...
    private fun generateNewIdentityKeys(p: android.content.SharedPreferences) {
        myKeyPair = E2EManager.generateECDHKeyPair()
        val priv = Base64.getEncoder().encodeToString(myKeyPair.private.encoded)
        val enc = E2EManager.encryptWithHardwareKey(priv)
        p.edit().apply {
            putString("my_public_key", E2EManager.publicKeyToString(myKeyPair.public))
    ...
    private fun initiateHandshake(p: Peer) {
        if (p.handshakePublicKey.isEmpty()) return
        if (handshakeLoading[p.onionAddress] == true) return
        handshakeLoading[p.onionAddress] = true
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val eKP = E2EManager.generateECDHKeyPair()
                val pPK = E2EManager.stringToPublicKey(p.handshakePublicKey)
                val sS = E2EManager.getSharedSecret(eKP.private, pPK)
                val sKBytes = com.p2p.torchat.crypto.HKDF.deriveKey(sS.encoded, null, "SessionKey".toByteArray(), 32)
                val sK = SecretKeySpec(sKBytes, "AES")
                val myID = E2EManager.publicKeyToString(myKeyPair.public)
                val data = "${E2EManager.publicKeyToString(eKP.public)}|PFS|$myID"
                val res =
                    p2pMessenger.sendPayloadOverTor(
                        p.onionAddress,
                        NetworkPayload(
                            PayloadType.SESSION_HANDSHAKE,
                            (torManager.torState.value as? TorState.Running)?.onionAddress ?: "",
                            p.onionAddress,
                            data,
                        ),
                ...
                if (res.isSuccess) sessionKeys[p.onionAddress] = sK
            } catch (e: Exception) {
            } finally {
    ...
    private fun startPresenceLoop(peers: MutableList<Peer>) {
        CoroutineScope(Dispatchers.IO).launch {
            while (true) {
                val my = (torManager.torState.value as? TorState.Running)?.onionAddress ?: ""
                if (my.isNotEmpty()) {
                    peers.forEach {
                        val ping = NetworkPayload(PayloadType.PING, my, it.onionAddress, "")
                        val res = p2pMessenger.sendPayloadOverTor(it.onionAddress, ping, timeoutMs = 15000)
                        if (res.isFailure) {
                            val idx = peers.indexOfFirst { p -> p.onionAddress == it.onionAddress }
                            if (idx != -1) peers[idx] = peers[idx].copy(isOnline = false)
                        }
                    }
                }
                delay(60_000L)
    ...
    private fun broadcastMyStatus(
        ...
        p.forEach {
            val pay = NetworkPayload(PayloadType.PONG, my, it.onionAddress, if (a) "ONLINE" else "OFFLINE")
    ...
    @Composable private fun AuthScreen(
        ...
        com.p2p.torchat.ui.screens.AuthScreen(mode = mode, attemptsLeft = attemptsLeft, onAuthSuccess = onAuthSuccess, onBack = onBack)
    }

    @Composable private fun SubscriptionScreen(
        onionAddress: String,
        onActivate: (String) -> Unit,
    ) {
        com.p2p.torchat.ui.screens.SubscriptionScreen(onionAddress = onionAddress, onActivate = onActivate)
    }
    ...
    @Composable private fun ChatScreen(
        ...
        com.p2p.torchat.ui.screens.ChatScreen(peer, messages, isHandshakeLoading, onSendMessage, onPickImage, onPickFile, onSaveAttachment, onDeleteSession, onOpenVerification, onBack)
    }

    @Composable private fun QRCodeScreen(
        onionAddress: String,
        myAlias: String,
        myPublicKey: String,
        onBack: () -> Unit,
    ) {
        com.p2p.torchat.ui.screens.QRCodeScreen(onionAddress, myAlias, myPublicKey, onBack)
    }

    @Composable private fun VerificationScreen(
        peer: Peer,
        onVerify: () -> Unit,
        onBack: () -> Unit,
    ) {
        com.p2p.torchat.ui.screens.VerificationScreen(peer, onVerify, onBack)
    }

    @Composable private fun ClientQRScannerScreen(
        onScanSuccess: (String) -> Unit,
        onBack: () -> Unit,
    ) {
        com.p2p.torchat.ui.screens.ClientQRScannerScreen(onScanSuccess, onBack)
    }
    ...
    @Composable private fun SettingsScreen(
        ...
        com.p2p.torchat.ui.screens.SettingsScreen(onExportBackup, onImportBackup, isAutoBackupEnabled, onToggleAutoBackup, onChangePassword, onExtendLicense, expiryDate, onOpenInfo, onOpenTerms, onBack)
    }

    @Composable private fun TermsOfUseScreen(
        isViewOnly: Boolean = false,
        onAccept: () -> Unit = {},
        onBack: () -> Unit = {},
    ) {
        com.p2p.torchat.ui.screens.TermsOfUseScreen(isViewOnly, onAccept, onBack)
    }

    @Composable private fun SeedScreen(
        mode: SeedMode,
        seed: List<String>,
        ...
        com.p2p.torchat.ui.screens.SeedScreen(mode, seed, onAction, onBack, onSkip, onRemoveSeed)
    }
}
