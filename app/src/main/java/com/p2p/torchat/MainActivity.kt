...
class MainActivity : ComponentActivity() {
    ...
    private fun AppContent() {
        ...
        val myOnionAddress = (torState as? TorState.Running)?.onionAddress ?: ""
        ...
        Surface(modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
            when (val screen = currentScreen) {
                ...
                is Screen.Subscription -> SubscriptionScreen(onionAddress = myOnionAddress, onActivate = { handleSubscription(it, myOnionAddress) })
                is Screen.Home -> HomeScreen(torState = torState, myOnionAddress = myOnionAddress, myAlias = myAlias, myPublicKey = E2EManager.publicKeyToString(myKeyPair.public), isDarkTheme = isDarkTheme, isAvailable = isAvailable, expiryDate = expiryDate, peers = peersList, unreadCounts = unreadCounts, peerToConfirm = peerToConfirmWithKey, onToggleTheme = { isDarkTheme = !isDarkTheme; saveThemePreference(isDarkTheme) }, onToggleAvailability = { isAvailable = !isAvailable; saveAvailabilityPreference(isAvailable); broadcastMyStatus(isAvailable, peersList) }, onUpdateMyAlias = { myAlias = it; saveMyAlias(it) }, onAddPeerDirect = { a, o, k -> handleAddPeer(a, o, k) }, onSelectPeer = { handleSelectPeer(it) }, onOpenQRCode = { currentScreen = Screen.QRCode }, onOpenQRScanner = { currentScreen = Screen.QRScanner }, onOpenSettings = { currentScreen = Screen.Settings }, onUpdateOnionAddress = { torManager.updateOnionAddress(sanitizeOnionAddress(it)) }, onDeletePeer = { peersList.remove(it); savePeers(peersList) }, onConfirmPeerHandled = { peerToConfirmWithKey = null })
                is Screen.Chat -> ChatScreenContent(screen.peer)
                is Screen.QRCode -> QRCodeScreen(onionAddress = myOnionAddress, myAlias = myAlias, myPublicKey = E2EManager.publicKeyToString(myKeyPair.public), onBack = { currentScreen = Screen.Home })
                ...
                is Screen.SeedRestore -> SeedScreen(mode = SeedMode.INPUT, seed = emptyList(), onAction = { handleSeedRestore(it) }, onBack = { currentScreen = Screen.Settings })
            }
            DialogsContent()
        }
    }

    @Composable private fun ChatScreenContent(p: Peer) { val ms = messagesMap.getOrPut(p.onionAddress) { mutableStateListOf() }; LaunchedEffect(p.onionAddress) { if (!sessionKeys.containsKey(p.onionAddress)) initiateHandshake(p) }; ChatScreen(p, ms, handshakeLoading[p.onionAddress] ?: false, { sendMessage(p, it, PayloadType.CHAT_MESSAGE, null, ms) }, { activeChatPeer = p; activeChatMessages = ms; pickImageLauncher.launch(PickVisualMediaRequest(ActivityResultContracts.PickVisualMedia.ImageOnly)) }, { activeChatPeer = p; activeChatMessages = ms; pickFileLauncher.launch("*/*") }, { f, b -> saveAttachmentToExternalStorage(f, b) }, { handleDeleteSession(p, ms) }, { currentScreen = Screen.Verification(p) }, { currentScreen = Screen.Home }) }
    @Composable private fun SeedBackupContent() { if (currentSeed.isEmpty() && getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).getString("persistent_backup_key", null) == null) currentSeed = MnemonicManager.generateMnemonic(); SeedScreen(SeedMode.DISPLAY, currentSeed, { exportBackupLauncher.launch("torchat_backup.json") }, { val p = getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE); if (isAuthenticated && (currentSeed.isNotEmpty() || p.getString("persistent_backup_key", null) != null)) currentScreen = Screen.Home else currentScreen = Screen.Settings }, { currentScreen = Screen.Home }, { handleRemoveSeed() }) }

    private fun handleAuthResult(password: String): Boolean {
        if (password == "WIPE_NOW") { performWipe(); return false }
        if (password.startsWith("CREATE:")) { try { val p = password.removePrefix("CREATE:").split("|"); val hash = E2EManager.hashPassword(p[0]); savePasswordHash(hash); savedPasswordHash = hash; torManager.updateOnionAddress(p[1]); completeAuth(); currentScreen = Screen.SeedBackup; return true } catch (e: Exception) { return false } }
    ...
    private fun handleSubscription(code: String, onion: String) { CoroutineScope(Dispatchers.IO).launch { val netTime = timeFetcher.fetchTimeViaTor() ?: System.currentTimeMillis(); val matchedDays = totpManager.findMatchingClientDuration(code, onion, netTime); if (matchedDays != null) { val newExpiry = maxOf(netTime, expiryDate) + (matchedDays.toLong() * 24 * 60 * 60 * 1000); saveExpiryDate(newExpiry); expiryDate = newExpiry; withContext(Dispatchers.Main) { currentScreen = Screen.Home; Toast.makeText(this@MainActivity, "ESTESO!", Toast.LENGTH_LONG).show() } } } }
    private fun handleAddPeer(alias: String, onion: String, pubKey: String) { val clean = sanitizeOnionAddress(onion); if (peersList.any { it.onionAddress == clean }) Toast.makeText(this, "GIÀ ESISTENTE", Toast.LENGTH_SHORT).show() else { peersList.add(Peer(clean, alias, pubKey)); savePeers(peersList) } }
    ...
    private fun handleDeleteSession(peer: Peer, messages: MutableList<Message>) { CoroutineScope(Dispatchers.IO).launch { p2pMessenger.sendPayloadOverTor(peer.onionAddress, NetworkPayload(PayloadType.SESSION_TERMINATE, (torManager.torState.value as? TorState.Running)?.onionAddress ?: "", peer.onionAddress, "TERMINATE")) }; messages.clear(); sessionKeys.remove(peer.onionAddress); handshakeLoading.remove(peer.onionAddress); currentScreen = Screen.Home }
    private fun handleRemoveSeed() { if (currentSeed.isNotEmpty()) { val salt = getOrCreateSalt(); val key = MnemonicManager.deriveKeyFromMnemonic(currentSeed, salt); val keyBase64 = Base64.getEncoder().encodeToString(key.encoded); getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().apply { putString("persistent_backup_key", keyBase64); remove("saved_seed"); apply() }; currentSeed = emptyList(); Toast.makeText(this, "SEED RIMOSSO", Toast.LENGTH_LONG).show() } }
    private fun handleExport(uri: Uri) { try { contentResolver.takePersistableUriPermission(uri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION or Intent.FLAG_GRANT_READ_URI_PERMISSION); getSharedPreferences("tor_chat_prefs", Context.MODE_PRIVATE).edit().putString("last_backup_uri", uri.toString()).apply(); contentResolver.openOutputStream(uri)?.use { it.write(backupManager.createEncryptedBackupJson(currentSeed, getOrCreateSalt()).toByteArray()) }; Toast.makeText(this, "Backup OK", Toast.LENGTH_SHORT).show() } catch (e: Exception) { } }
            ...
            PayloadType.CHAT_MESSAGE, PayloadType.IMAGE, PayloadType.FILE -> {
                if (isAvailable) try {
                    ...
                    if (sK != null) {
                        ...
                        notificationHelper.showChatNotification(peersList.find { it.onionAddress == payload.senderOnion }?.alias ?: "Peer", if (payload.type == PayloadType.IMAGE) "Foto" else if (payload.type == PayloadType.FILE) "File" else dec)
                    }
    ...
    private fun handleHandshakeReceived(p: NetworkPayload) {
        try {
            ...
            val pK = E2EManager.stringToPublicKey(pts[0]); val sK = E2EManager.decrypt(pts[1], E2EManager.getSharedSecret(myKeyPair.private, pK))
            sessionKeys[p.senderOnion] = SecretKeySpec(Base64.getDecoder().decode(sK), "AES"); handshakeLoading[p.senderOnion] = false
    ...
    private fun handleAcceptConnection(req: NetworkPayload) {
        ...
        try { val pK = E2EManager.stringToPublicKey(p[0]); val sK = E2EManager.decrypt(p[1], E2EManager.getSharedSecret(myKeyPair.private, pK)); sessionKeys[req.senderOnion] = SecretKeySpec(Base64.getDecoder().decode(sK), "AES") } catch (e: Exception) { }
    ...
    private fun getOrCreateSalt(): ByteArray { val a = MasterKeys.getOrCreate(MasterKeys.AES256_GCM_SPEC); val p = EncryptedSharedPreferences.create("secure_prefs", a, this, EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV, EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM); val s = p.getString("install_salt", null); return if (s != null) Base64.getDecoder().decode(s) else { val salt = ByteArray(16).apply { SecureRandom().nextBytes(this) }; p.edit().putString("install_salt", Base64.getEncoder().encodeToString(salt)).apply(); salt } }
